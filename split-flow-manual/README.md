...
```
$ fodt-split-main --filename=Manual.fodt --maindir=out
```
...
To split a chapter further into subsections, run the command:

```
$ fodt-split-subdocument --maindir=out --subdocument=4 --subsection=3
```

This will split chapter 4, section 3 into subsection documents (subsection 4.3.1, 4.3.2, ...)
...
in the subsection which is used to determine the subsubsection number of the added keyword.

For the above example, adding the keyword involves updating the sub document `out/chapters/4.fodt`
to include the new file `out/chapters/subsections/HELLO.fodt`.
